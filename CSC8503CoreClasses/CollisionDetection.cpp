...
bool CollisionDetection::RayBoxIntersection(const Ray& r, const Vector3& boxPos, const Vector3& boxSize, RayCollision& collision) {
	...
	for (int i = 0; i < 3; i++) // 3 best intersections
	{
		if (rayDir[i] > 0)
		{
	...
	// now perform the box test
	Vector3 intersection = rayPos + (rayDir * bestT);
	const float epsilon = 0.0001f; // leeway in our calcs
	for (int i = 0; i < 3; i++)
	{
...
bool CollisionDetection::RaySphereIntersection(const Ray& r, const Transform& worldTransform, const SphereVolume& volume, RayCollision& collision) {
	...
	// next project sphere origin onto the ray direction vector
	...
	// get closest point on the ray line to the sphere
...
bool CollisionDetection::RayCapsuleIntersection(const Ray& r, const Transform& worldTransform, const CapsuleVolume& volume, RayCollision& collision) {
	...
	Vector3 offset = (orientation * Vector3(0.0f, 1.0f, 0.0f) * (volume.GetHalfHeight() - volume.GetRadius()));
	Vector3 capsuleTop = position + offset;
	Vector3 capsuleBottom = position - offset;

	Vector3 pointC = position + Vector3::Cross(capsuleTop - position, position - r.GetDirection().Normalised());

	Plane p = Plane::PlaneFromTri(position, capsuleTop, pointC);
	...
	Vector3 vA_1 = rayPoint - capsuleTop;	
	Vector3 vB_1 = capsuleTop - position;
	Vector3 vA_2 = rayPoint - capsuleBottom;
	Vector3 vB_2 = capsuleBottom - position;

	Transform tempSphere = worldTransform;
	if (Vector3::Dot(vA_1, vB_2) < 0.0f)
	{
		tempSphere.SetPosition(capsuleTop);
		return RaySphereIntersection(r, tempSphere, (SphereVolume&)volume, collision);
	}
	else if (Vector3::Dot(vA_2, vB_2) > 0.0f)
	{
		tempSphere.SetPosition(capsuleBottom);
		return RaySphereIntersection(r, tempSphere, (SphereVolume&)volume, collision);
	}
	else
	{
		float projection = Vector3::Dot(rayPoint - position, orientation * Vector3(0.0f, 1.0f, 0.0f));
		Vector3 d = position + orientation * Vector3(0.0f, 1.0f, 0.0f) * projection;
		if ((d - position).Length() > volume.GetRadius()) return false;
		tempSphere.SetPosition(d);
		return RaySphereIntersection(r, tempSphere, (SphereVolume&)volume, collision);
	}
	return false;
}
...
bool CollisionDetection::ObjectIntersection(GameObject* a, GameObject* b, CollisionInfo& collisionInfo) {
	// determine which collision detection function to use
...
bool  CollisionDetection::OBBSphereIntersection(const OBBVolume& volumeA, const Transform& worldTransformA,
	...
	Transform tempAABB;
	AABBVolume tempAABBVolume(volumeA.GetHalfDimensions());

	Transform tempSphere;
	tempSphere.SetPosition(worldTransformA.GetOrientation().Conjugate() * delta);

	if (AABBSphereIntersection(tempAABBVolume, tempAABB, volumeB, tempSphere, collisionInfo))
	{
...
bool CollisionDetection::SphereCapsuleIntersection(
	...
	Matrix3 capsuleOrientation = Matrix3(worldTransformA.GetOrientation());
	Vector3 capsuleOffset = (capsuleOrientation * Vector3(0.0f, 1.0f, 0.0f) * (volumeA.GetHalfHeight() - volumeA.GetRadius()));
	Vector3 capsuleTop = capsulePosition + capsuleOffset;
	Vector3 capsuleBottom = capsulePosition - capsuleOffset;

	Vector3 spherePosition = worldTransformB.GetPosition();

	float projection = Vector3::Dot(capsuleTop - capsuleBottom, spherePosition - capsulePosition);
	Vector3 d = capsulePosition + capsuleOrientation * Vector3(0.0f, 1.0f, 0.0f) * projection;

	Transform tempSphere = worldTransformA;
	tempSphere.SetPosition(d);


	return SphereIntersection(SphereVolume(volumeA.GetRadius()), tempSphere, volumeB, worldTransformB, collisionInfo);
	*/
	return false;
}
